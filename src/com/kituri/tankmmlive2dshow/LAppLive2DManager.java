...
public class LAppLive2DManager
{
	...
	public void update(GL10 gl)
	{
		...
		if(reloadFlg)
		{
			...
			int no = modelCount % LAppDefine.MODELS.size();

			String model = "live2d/" + LAppDefine.MODELS.get(no)
					+ "/" + LAppDefine.MODELS.get(no) + ".json";
					//"live2d/type59_a/type59_a.json"
			Log.i("", "model:" + model);
			try {
				//models.get(0).deepFile(SampleApplication.getContext(), "live2d");
				releaseModel();
				models.add(new LAppModel());
				models.get(0).load(gl, model);
				models.get(0).feedIn();
			} catch (Exception e) {
